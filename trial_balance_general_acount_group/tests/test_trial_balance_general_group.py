...
class TestTrialBalanceGeneralGroup(TransactionCase):
    def test_report_action_is_defined(self):
        self.assertTrue(
            self.env.ref('trial_balance_general_acount_group.action_report_group_general_trial_balance')
        )

    def test_wizard_returns_report_action(self):
        wizard = self.env['trial.balance.general.group'].create({
        ...
        self.assertEqual(
            ...
            'trial_balance_general_acount_group.report_group_general_trial_balance',
        )
